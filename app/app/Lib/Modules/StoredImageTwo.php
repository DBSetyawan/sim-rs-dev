...
class StoredImageTwo implements StoredDocumentTWOInterface
{
    ...
    public function StoredDocumentComponentOrdersTWO(Object $request, $generateID, $generateIDNEXT) {
        $data = array();
                        ...
                        for ($x = 0; $x < $request->TotalFiles; $x++) 
                            {
                                if ($request->hasFile('files'.$x)) 
                                {
                                    ...
                                    $file->component = $request->cmpcz2;
                                    $file->design_plan = isset($request->lastIndex) ? "cp".$request->lastIndex : $request->design2;
