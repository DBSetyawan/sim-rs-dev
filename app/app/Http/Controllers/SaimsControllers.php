...
class SaimsControllers extends Controller
{
    ...
    public function detailPurchaseInvoiceH(Request $request) {
        ...
        public function reports($rpt, $checksum)
        {
            ...
            $data = ['name' => Str::of($rpt)->pipe('md5')->prepend('')];
                $pdf = PDF::loadView('saimanual.print' , compact('tr','ppn','total_akhir','phone_cs','fax_cs',
                'ship_customer','ship','total_rslt','brgdt','cek_isi_PO','Tanggal','tanggal_jatuh_tempo','todatetimeStringcreatedSAI',
                'npwp','TOP','TAX','no_SOD','detailrows_sai','dt_live_sim_GID','PO','customers','emailcs'));
    ...
    public function getDetailSalesInvoice(Request $request){
        ...
        return response()->json(['tb_po' => $tbPO, 
