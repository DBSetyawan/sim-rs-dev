<html lang="en"><head>
    ...
    <link href="{{ URL::asset('argon') }}/vendor/@fortawesome/fontawesome-free/css/all.min.css" rel="stylesheet">
    <!-- Argon CSS -->
        ...
        <li class="nav-item dropdown">
            <a class="nav-link" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <div class="media align-items-center">
                    ...
                    <img alt="Image placeholder" src="{{ URL::asset('argon') }}/img/logo34.png">
                    </span>
                </div>
            </a>
...
<div class="container-fluid">
    <!-- Brand -->
    <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="{{ route('home') }}">FNC CONTROL</a>
    <!-- Form -->
    ...
    <ul class="navbar-nav align-items-center d-none d-md-flex">
        <li class="nav-item dropdown">
            <a class="nav-link pr-0" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <div class="media align-items-center">
                    <span class="avatar avatar-sm rounded-circle">
                        <img alt="Image placeholder" src="{{ URL::asset('argon') }}/img/theme/team-4-800x800.jpg">
                    </span>
            ...
            <div class="card shadow">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">SAIM manually</h3>
                        </div>
                        <div class="col-4 text-right">
                            @role('head finance|staff finance|manager finance')
                                <a href="{{ route('saims.create') }}" class="btn btn-sm btn-success">tambah saim manual</a>
                            @endrole
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                          @foreach($sm as $users)
                                <tr>
                                    <td>{{ $users->sai }}</td>
                                    <td>{{ $users->NamaCus }}</td>
                                    @if ($users->status_docs == "open")
                                        <td><span class="badge badge-primary">{{ $users->status_docs }}</span></td>
                                        <td class="text-right">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                    @role('manager finance|head finance|staff finance')
                                                        @if($users->status_docs == "approved")
                                                                <a class="dropdown-item" href=" {{ route('saims.reports', ['rpt' => $users->sai, 'checksum'=> \Str::of($users->sai)->pipe('md5') ]) }}">Print</a>
                                                            @else
                                                                <a class="dropdown-item" href="{{ route('saims.delete', ['id'=> $users->id]) }}">Hapus</a>
                                                                <a class="dropdown-item" href="{{ route('saims.edit', ['id'=> $users->id, 'sai' => $users->sai]) }}">Ubah</a>
                                                            @endif
                                                        @else
                                                            <a class="dropdown-item badge badge-danger" href="#stoped">Maaf anda tidak hak akses untuk fitur ini</a>
                                                    @endrole
                                                </div>
                                            </div>
                                        </td>
                                    @else
                                        @if ($users->status_docs == "approved")
                                                <td><span class="badge badge-success">{{ $users->status_docs }}</span></td>
                                            @else  
                                                <td><span class="badge badge-warning">{{ $users->status_docs }}</span></td>
                                        @endif
                                            <td class="text-right">
                                                <div class="dropdown">
                                                    <a class="btn btn-sm btn-icon-only text-light" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
