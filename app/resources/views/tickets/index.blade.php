                ...
                <tbody>
                  @forelse ($tickets as $rx => $customers)
                          <tr>
                            ...
                            <td class="px-5 py-5 w-4/5 bg-white text-sm @if (!$loop->last) border-gray-200 border-b @endif text-center text-lg">
                                <a class="text-center text-lg" href="{{ route('show.tickets', ['ticket_id' => $customers->ticket_id ]) }}">
                              ...
                              <td class="px-5 py-5 bg-white text-sm w-1/6 @if (!$loop->last) border-gray-200 border-b @endif text-right text-lg">
                                  @php
                                ...
                                <td class="px-5 py-5 w-1/6 bg-white text-sm @if (!$loop->last) border-gray-200 border-b @endif text-right">
                                    @if($customers->status === 'Open')
                                    <div class="flex items-center justify-center mb-3">
                                        <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                                            <a class="text-base bg-gray-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" href="{{ route('show.tickets', ['ticket_id' => $customers->ticket_id ]) }}" class="btn btn-primary">Comment</a>
                                        </div>
                                      </div>
                                      <div class="flex items-center justify-center mb-3">
                                        <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                                            <form action="{{ route('close_ticket.tickets', ['ticket_id' => $customers->ticket_id ]) }}" method="POST">
                                                {!! csrf_field() !!}
                                                <button type="submit" class="text-base bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Close</button>
                                            </form>
                                        </div>
                                      </div>
                                    @endif
...
<span class="tool-_disclaimer" style="color: #8189a7;font-size: 13px;
  ...
  <div style="margin-bottom: 4300px;"></div>
<footer id="footer">
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-12">
        <div class="contact-details">
          <h4>Contact Us</h4>
          <ul class="contact">
            <li><p><i class="fa fa-map-marker"></i> <strong>Address:</strong> <span id="index_tickets_address_footer"></p></li>
            <li><p><i class="fa fa-phone"></i> <strong>Phone:</strong> <span id="index_tickets_phone_footer"></p></li>
            <li><p><i class="fa fa-fax"></i> <strong>Fax:</strong> <span id="index_tickets_fax_footer"></p></li>
            <li><p><i class="fa fa-envelope"></i> <strong>Email:</strong> <a href="mailto:printing@krisanthium.com"> <span id="index_tickets_email_footer"></a></p></li>
            <li><p><i class="fa fa-external-link-square"></i> <strong>URL:</strong> <a href="{{ route('dashboard') }}"> <span id="index_tickets_url_footer"></a></p></li>
          </ul>
        </div>
      </div>
    </div>
...
RenderAPIAboutCompany().then(function(data){
      $('#index_tickets_address_footer').append(data.data.address)
      $('#index_tickets_phone_footer').append(data.data.phone)
      $('#index_tickets_fax_footer').append(data.data.Fax)
      $('#index_tickets_email_footer').append(data.data.Email)
      $('#index_tickets_url_footer').append(data.data.URL)
  });
